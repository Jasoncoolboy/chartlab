...
- [ ] In `timeX`, snap an off-grid time to the bar that CONTAINS it (binary search over the current timeframe's time array:
      the last bar with time ≤ t), so intrabar fills and zones draw on every timeframe button.
- [ ] Until then, have `setups_from_rows` warn (or offer `snap=True`) when a row's times are not on the `--timeframe` grid.
- [ ] Test: a row with an off-grid entry and a zone that starts before the window must produce a drawn trade and zone
...
- [ ] Add a unit to the trade/row (`net_unit`: "$" default, "R", "pips") and format with sensible decimals for non-dollar units.
...
- [ ] Optionally build a page's timeframes from M1 (aggregate on the source clock, then convert — the approach
...
- [ ] Open a setup page zoomed to its own window (the `pre`/`post` bars), e.g. an initial visible range in the spec.
...
- [ ] Clip the label to the box width (or truncate with an ellipsis and show the full text in a tooltip).
...
- [ ] Charge at 00:00 server (convert with `sources.utc_to_ftmo_server`) and triple the Wed→Thu rollover; test both clocks.
- [ ] Note for users of saved cost files: a JSON without `commission_pct_side` now gets the 0.0007 % default ADDED to its flat
